...
t1 = Sys.time()
result <- 
  foreach(B=1:25,.options.snow=opts) %dopar% {
  tryCatch({
    library(smoother)
    library(MASS)
    library(zoo)
    ...
    fr_vec = rep(c(0.5, 0.2), c(25, P-25)) # two sets of flat firing rate: first set is 5, second set is 2
    for(trial in 1:P){
      true_fr[trial,]=fr_vec[trial]*10  
    ...
    set.seed(B)
    cat('seed: ', B, '\n')
    sim = simulate.BP3(T, P, gam, sd, max_fr, fr_vec)
...
close(pb)
stopCluster(cl)
save.image("./result/20240716_flat_sim_25seeds.RData")
